...
const toLastNames = people => {
  ...
  people.map((i)=>
  {
    newarr1.push(i.firstName+" "+ i.lastName);
  });
  return newarr1;};
...
const addValues = (arr) => {
  const sum = arr.reduce(function summarize(sum, num) {
...
const addPurchases = (arr) => {
  let newsum=arr.reduce(function(x,y)
  {
    x=x+y.purchasePrice;
...
const countNumberOfElements = (arr) => {
  let counting =arr.reduce(function(x,y,z)
  {
    x=z+1;
    return x;
    },0);
...
const returnNames = (arr) => {
  let data = arr.reduce(function(x,y,i)
  {
    x[i]=(y.name);
