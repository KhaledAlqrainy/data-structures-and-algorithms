...
const toTitleCase = (arr) => 
{
  ...
  let a = arr.map(string => string[0].toUpperCase() + string.slice(1) );
  return a;
};
...
let biggerThanLuke = (arr) => {
  ...
  let luke1 = Number(arr.filter(character => /luke/i.test(character.name))[0].mass);

  return arr.reduce((arr, char) =>
   {
    if (Number(char.mass) > luke1) arr.push(char.name);
