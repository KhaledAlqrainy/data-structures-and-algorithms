...
const longestString = (arr) => {
  let newarr1 = [...arr];
  let long = newarr1.sort((a, b) => b.length - a.length);
  return arr.indexOf(long[0]);};
...
const findHappiness = (arr) => {
  ...
  let newarr1 = arr.filter((item) => item.includes(':)'));
  return newarr1;
};
...
const allHappy = (arr) => 
{
  ...
  if (newbool.includes('false'))
   {
    return false;
  } else if (!newbool.includes('false'))
   {
    return true;
  }
